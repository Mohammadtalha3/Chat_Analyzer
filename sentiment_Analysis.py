...
def preprocessing(data,key):
    ...
    with open(data, 'r', encoding='utf-8') as raw_data:
      ...
      message= re.split(split_formats[key],raw_string)[1:]
      dates= re.findall(split_formats[key],raw_string)

      dt=pd.DataFrame({'messages': message,'dates':dates})
...
data = 'D:\chat_Analyzer2\Dataset\whatsapp-chat-data.txt'  
key = '12hr'
